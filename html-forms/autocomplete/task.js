class Autocomplete {
  ...
  getMatches( text ) {
    const select = document.querySelector(".autocomplete__input")
    const requered = Array.from(select.options).reduce((ac,element)=>{
      if(element.innerText.includes(text)) {
        ac.push({value : element.value , text : element.innerText})
      }
      return ac
     },[])
    return requered;
